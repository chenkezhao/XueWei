...
package com.xuewei.facebook.zoomable;
...
import com.xuewei.facebook.gestures.TransformGestureDetector;
...
public class DefaultZoomableController implements ZoomableController , TransformGestureDetector.Listener {
	 ...
	 * just return the internal flag directly. However, this requires interface
	 * change and negation of meaning.
	 */
	@Override
	 ...
	 * view-absolute coordinates. The zoomable transformation is taken into
	 * account.
	 *
	...
	/**
	 * Maps point from view-absolute to image-relative coordinates. This takes
	 * into account the zoomable transformation.
	 */
	...
	/**
	 * Maps point from image-relative to view-absolute coordinates. This takes
	 * into account the zoomable transformation.
	 */
	 ...
	 * This does NOT take into account the zoomable transformation. Points are
	 * represented by a float array of [x0, y0, x1, y1, ...].
	 *
	 * @param destPoints
	 * destination array (may be the same as source array)
	 * @param srcPoints
	 * source array
	 * @param numPoints
	 ...
	 * image point corresponds to the given view point.
	 *
	 * @param scale
	 * desired scale, will be limited to {min, max} scale factor
	 * @param imagePoint
	 * 2D point in image's relative coordinate system (i.e. 0 <= x, y <= 1)
	 * @param viewPoint
	 * 2D point in view's absolute coordinate system
	 */
	 ...
	 * and position the image so that the given image point corresponds to the
	 * given view point.
	 *
	 * @param outTransform
	 * the matrix to store the result to
	 * @param scale
	 * desired scale, will be limited to {min, max} scale factor
	 * @param imagePoint
	 * 2D point in image's relative coordinate system (i.e. 0 <= x, y <= 1)
	 ...
	 * 2D point in view's absolute coordinate system
	 * @param limitFlags
	 * whether to limit translation and/or scale.
	...
	public void onGestureBegin(TransformGestureDetector detector) {
		...
		// We only received a touch down event so far, and so we don't know yet
		// in which direction a
		// future move event will follow. Therefore, if we can't scroll in all
		// directions, we have to
		// assume the worst case where the user tries to scroll out of edge,
		// which would cause
	 ...
	 *
	 * <p>
	 * The image is attempted to be centered within the view bounds if the
	 * transformed image is smaller. There will be no empty spaces within the
	 * view bounds if the transformed image is bigger. This applies to each
	 * dimension (horizontal and vertical) independently.
	 *
	 * @param limitTypes
	 * whether to limit translation along the specific axis.
	 * @return whether limiting has been applied or not
	 */
	...
	/**
	 * Checks whether the specified limit flag is present in the limits
	 * provided.
	 *
	 * <p>
	 * If the flag contains multiple flags together using a bitwise OR, this
	 * only checks that at least one of the flags is included.
	 *
	 * @param limits
	 * the limits to apply
	 * @param flag
	 * the limit flag(s) to check for
	...
	/**
	 * Returns the offset necessary to make sure that: - the image is centered
	 * within the limit if the image is smaller than the limit - there is no
	 * empty space on left/right if the image is bigger than the limit
	 */
	private float getOffset(float imageStart, float imageEnd, float limitStart, float limitEnd, float limitCenter) {
		float imageWidth = imageEnd - imageStart,
				limitWidth = limitEnd - limitStart;
	...
	/**
	 * Gets the scale factor for the given matrix. This method assumes the equal
	...
	private boolean isMatrixIdentity(Matrix transform, float eps) {
		// Checks whether the given matrix is close enough to the identity
		// matrix:
		// 1 0 0
		// 0 1 0
		// 0 0 1
